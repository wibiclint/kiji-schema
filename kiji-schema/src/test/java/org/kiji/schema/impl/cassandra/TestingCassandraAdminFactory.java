...
public final class TestingCassandraAdminFactory implements CassandraAdminFactory {
  /** Singleton. */
  //private static final CassandraAdminFactory DEFAULT = new TestingCassandraAdminFactory();
  // TODO: Discuss with Milo whether this style is okay for a Singleton that needs to be passed a parameter.
  private static TestingCassandraAdminFactory DEFAULT = null;

  /** @return an instance of the default factory. */
  public static CassandraAdminFactory get(Session session) {
    if (DEFAULT == null) {
      DEFAULT = new TestingCassandraAdminFactory(session);
    } else {
      assert(DEFAULT.session == session);
    }
    return DEFAULT;
  }
  ...
  public CassandraAdmin create(KijiURI uri) throws IOException {
    ...
    return TestingCassandraAdmin.makeFromKijiURI(session, uri);
  }
}
